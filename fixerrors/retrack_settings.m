...
% Last Modified by GUIDE v2.5 14-Dec-2016 20:41:43
...
if isfield( handles.fixhandles, 'temp_arena' )
   if isfield(handles.fixhandles.temp_arena, 'center_x')
      [cc, rr] = meshgrid( handles.c0:handles.c1, handles.r0:handles.r1 );
      non_arena_mask = sqrt( ...
      ...
      handles.im(:,(handles.c0:handles.c1) < handles.fixhandles.temp_arena.upper_left(1)) = 0;
      handles.im(:,(handles.c0:handles.c1) > handles.fixhandles.temp_arena.lower_right(1)) = 0;
      handles.im((handles.r0:handles.r1) < handles.fixhandles.temp_arena.upper_left(2),:) = 0;
      handles.im((handles.r0:handles.r1) > handles.fixhandles.temp_arena.lower_right(2),:) = 0;
   end
end
...
elseif isfield( handles, 'choosefg' ) && handles.choosefg && isfield( handles, 'choosefgpt1' )
   if handles.circularfg
      ...
      rect_x = [handles.choosefgpt1(1), x, x, handles.choosefgpt1(1), handles.choosefgpt1(1)];
      rect_y = [handles.choosefgpt1(2), handles.choosefgpt1(2), y, y, handles.choosefgpt1(2)];
      set(handles.hchoosefg, 'xdata', rect_x, 'ydata', rect_y);
   end
end
...
if isfield( handles, 'choosefg' ) && handles.choosefg && isfield( handles, 'choosefgpt1' )
   ...
   if handles.circularfg
      ...
      arena.upper_left = [min([handles.choosefgpt1(1), pt(1,1)]) + handles.c0, min([handles.choosefgpt1(2), pt(1,2)]) + handles.r0];
      arena.lower_right = [max([handles.choosefgpt1(1), pt(1,1)]) + handles.c0, max([handles.choosefgpt1(2), pt(1,2)]) + handles.r0];
   end
...
% --- Executes on button press in circularfg_checkbox.
function circularfg_checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to circularfg_checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.circularfg = get(hObject, 'value');
if isfield( handles.fixhandles, 'temp_arena' )
   handles.fixhandles = rmfield( handles.fixhandles, 'temp_arena' );
end
handles = ShowCurrentFrame(handles);
