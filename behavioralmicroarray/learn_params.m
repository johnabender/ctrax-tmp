...
for i = 1:nmovies,
  ...
  for j = 1:length(fliestolabel{i}),
    ...
    if ~isempty(labeledbehavior{i}(fly).starts) && labeledbehavior{i}(fly).starts(1) <= t0,
      ...
      fprintf('Shortening to frames %d-%d.\n',t0,t1);
...
for fly = 1:length(datalearn),
  ...
  datalearn(fly).nframes = min( [datalearn(fly).nframes, length( datalearn(fly).x )] );
  ...
  labels(fly).starts(labels(fly).starts < 1) = 1;
  labels(fly).ends = labels(fly).ends - t0 + 1;
  labels(fly).ends(labels(fly).ends > length( datalearn(fly).x )) = length( datalearn(fly).x );
end

%% choose file to save params to

fprintf('Choose a file to save the learned behavior parameters in.\n');
if ~exist('ds','var')
  ds = datestr(now,30);
end
hasmissed = false;
while true,
  ...
  helpmsg = 'Choose .mat file to save the learned behavior classifier parameters in';
  [paramsmatname,paramsmatpath] = uiputfilehelp('.mat','Choose file to save parameters in',paramsmatname,'helpmsg',helpmsg);
...
for fly = 1:nflies,
   % length of data somehow changes during preceding, so nframes may need to be updated
   datalearn(fly).nframes = min( [datalearn(fly).nframes length( datalearn(fly).x )] );
  ...
  if ~any(idxplotother),
    ...
    % plot labels
    for i = 1:length(labels(fly).starts),
      hlabel = plot(datalearn(fly).x(labels(fly).starts(i):labels(fly).ends(i)),...
        datalearn(fly).y(labels(fly).starts(i):labels(fly).ends(i)),...
        'm-','linewidth',8);
    end
    
    % plot the fly's position
    hmain = plot(datalearn(fly).x(idxplotother),datalearn(fly).y(idxplotother),'k.-');
    
    % plot detected positions
    for i = 1:length(seg.t1),
      hdetect = plot(datalearn(fly).x(seg.t1(i):seg.t2(i)),...
        datalearn(fly).y(seg.t1(i):seg.t2(i)),'c.-');
      plot(datalearn(fly).x(seg.t1(i)),...
        datalearn(fly).y(seg.t1(i)),'go','markerfacecolor','g');
      plot(datalearn(fly).x(seg.t2(i)),...
        datalearn(fly).y(seg.t2(i)),'ro','markerfacecolor','r');
    end
