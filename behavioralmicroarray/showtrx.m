...
if length(varargin) >= 1,
  ...
  if ~isempty(showtrxlastmovie),
    [handles.moviename, handles.pathname] = ...
      uigetfilehelp({'*.fmf';'*.sbfmf';'*.ufmf';'*.avi';'*.*'},'Choose Movie Name',...
      showtrxlastmovie,'helpmsg',helpmsg);
  else
    [handles.moviename, handles.pathname] = ...
      uigetfilehelp({'*.fmf';'*.sbfmf';'*.ufmf';'*.avi';'*.*'},'Choose Movie Name','helpmsg',helpmsg);
  end
...
set(handles.zoomlocaxes,'Clim',[0,maxv]);
...
if ~handles.isplaying,
  ...
  f = handles.f + f_skip;
  tic;
  while f >= 1 && f <= handles.nframes
    handles.f = f;    
    UpdatePlot(handles, hObject);
    SetFrameNumber(handles);
    spf = 1 / handles.fps;
    t = toc;
    pause( max( [spf - t, 0.01] ) )
    tic;
    if spf > t
       f = f + f_skip;
    else
       f = f + f_skip*round( (t - spf)/spf );
    end
...
% --- Executes on button press in reverse_playback.
function reverse_playback_Callback(hObject, eventdata, handles)
% hObject    handle to reverse_playback (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.reverse_playback = get(hObject,'Value');
guidata( hObject, handles );
