...
handles.nr = sz(1);
handles.nc = sz(2);
if length( sz ) == 3 && sz(3) == 3 % image is MxNx3
   handles.him = image( uint8( im ) );
else % assume it's MxN or MxNx1
...
if ~handles.isplaying,
  ...
  f = handles.f + f_skip;
  tic;
  while f >= handles.trx(handles.fly).firstframe && f <= handles.trx(handles.fly).endframe,
    handles.f = f;    
    handles = UpdatePlot(handles);
    SetFrameNumber(handles);
    spf = 1 / handles.fps;
...
% --- Executes on button press in reverseplayback.
function reverseplayback_Callback(hObject, eventdata, handles)
% hObject    handle to reverseplayback (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
