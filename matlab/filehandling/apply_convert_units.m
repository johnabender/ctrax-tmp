...
if isfield( trx, 'timestamps' )
   if isfield( trx, 'fps' )
      % only modify timestamps if fps has actually changed
      testfps = 1/mean( diff( trx(1).timestamps ) );
      if abs( fps - testfps ) > 1e-3
         for fly = 1:length(trx),
            trx(fly).timestamps = (trx(fly).firstframe - 1:trx(fly).endframe - 1)/trx(fly).fps;
         end
         didsomething = true;
      end
   end
