...
def known_extensions():
    return ['.fmf', '.avi', '.sbfmf', '.ufmf']
from params_030 import params, diagnostics
import ellipsesk_030 as ell
...
class AnnotationFile:
    ...
    def common_initialization( self ):
        ...
        self.buffer = []
        self.nbuffer = max( params.anndata_nbuffer,
                            self.maxlookback ) + 1
    ...
    def get_frame(self,f):
        ...
        #if DEBUG_LEVEL > 0: print "get_frame %d, framestracked = [%d,%d], framesbuffered = [%d,%d], frameswritten = [%d,%d]"%(f,self.firstframetracked,self.lastframetracked,self.firstframebuffered,self.lastframebuffered,self.firstframewritten,self.lastframewritten)
        if f < self.firstframebuffered or f > self.lastframebuffered:
            ...
            if DEBUG_LEVEL > 0: print "i = " + str(i)
    ...
    def finish_writing(self):
        ...
        for f in range(self.lastframewritten+1,self.lastframetracked+1):
            if self.islookupframe(f):
                self.file.flush()
                self.lookup.append(self.file.tell())
            if DEBUG_LEVEL > 1: print "Writing frame %d at %x: "%(f,self.file.tell()) + str(self.buffer[f-self.firstframebuffered])
            if not self.file.closed:
